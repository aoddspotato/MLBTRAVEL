#' getMLBtravel
#'
#' @param example SET TRUE to do example day where teams traveled rather than today's games
#' @export
getMLBtravel <- function(example=FALSE){
  ###########################################################################################
  ...
  if(isTRUE(example)){
    #do example day where teams traveled
    dtoday <- "20250630" ###example
    dyesterday <- "20250629" ###example
  }else{
    #automatically do for yesterday to today's games
    dtoday <- gsub("-","",as.character(Sys.Date()))
    dyesterday <- gsub("-","",as.character(Sys.Date()-1))
  }
